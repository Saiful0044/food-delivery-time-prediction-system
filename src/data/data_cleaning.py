...
def time_of_day(hour_series):
    return (
        pd.cut(hour_series, bins=[0,6,12,17,20,24], right=True, labels=["after_midnight","morning","afternoon","evening","night"])
    )
...
def data_cleaning(data: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .drop(index=six_star_index)     # drop invalid 6-star
...
def clean_lag_long(data: pd.DataFrame, threshold: float=1.0)-> pd.DataFrame:
...
if __name__=='__main__':
    ...
    perform_data_cleaning(data=data, saved_data_path=cleaned_data_save_path)
